...
Equation Labels are denoted by [Paper ID]-(Equation ID) format.
...
from ddpm.components.nn import mean_flat
from ddpm.components.losses import normal_kl, discretized_gaussian_log_likelihood
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        alphas = 1.0 - betas  # [1]: Page 2, above (4)
        self.alphas_cumprod = np.cumprod(
            alphas, axis=0
        )  # [1]: Page 2, above (4), \bar{alpha}
        self.alphas_cumprod_prev = np.append(1.0, self.alphas_cumprod[:-1])
        self.alphas_cumprod_next = np.append(self.alphas_cumprod[1:], 0.0)
        assert self.alphas_cumprod_prev.shape == (self.num_timesteps,)

        # calculations for diffusion q(x_t | x_{t-1}) and others
        self.sqrt_alphas_cumprod = np.sqrt(self.alphas_cumprod)  # [1]-(4)
        self.sqrt_one_minus_alphas_cumprod = np.sqrt(
            1.0 - self.alphas_cumprod
        )  # [1]-(4)
        self.log_one_minus_alphas_cumprod = np.log(
            1.0 - self.alphas_cumprod
        )  # [1]-(4), taking log for numerical stability
        self.sqrt_recip_alphas_cumprod = np.sqrt(1.0 / self.alphas_cumprod)  # [1]-(11)
        ...
        )  # [1]-(7): for forward process posterior
